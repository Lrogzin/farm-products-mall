...
import com.nuzoul.admin.service.ItemService;
...
import com.nuzoul.dto.ItemDto;
import com.nuzoul.mapper.TbItemCatMapper;
import com.nuzoul.mapper.TbItemDescMapper;
import com.nuzoul.mapper.TbItemMapper;
import com.nuzoul.pojo.TbItem;
import com.nuzoul.pojo.TbItemCat;
import com.nuzoul.pojo.TbItemDesc;
...
/**
 * Created by Nuzoul on 2017/7/29.
 */
@Service
public class ItemServiceImpl implements ItemService {
    ...
    public TbItem alertItemState(Long id, Integer state) {
        ...
        if (tbItemMapper.updateByPrimaryKey(tbMember) != 1){
            throw new FPMallException("修改商品状态失败");
        }
    ...
    public int deleteItem(Long id) {
        ...
        if(tbItemMapper.deleteByPrimaryKey(id)!=1){
            throw new FPMallException("删除商品失败");
        }
        if(tbItemDescMapper.deleteByPrimaryKey(id)!=1){
            throw new FPMallException("删除商品详情失败");
        }
        //发送消息同步索引库
        try {
    ...
    public TbItem addItem(ItemDto itemDto) {
        ...
        if(tbItemMapper.insert(tbItem)!=1){
            throw new FPMallException("添加商品失败");
        }
        ...
        if(tbItemDescMapper.insert(tbItemDesc)!=1){
            throw new FPMallException("添加商品详情失败");
        }
        //发送消息同步索引库
        try {
    ...
    public TbItem updateItem(Long id, ItemDto itemDto) {
        ...
        if(tbItemMapper.updateByPrimaryKey(tbItem)!=1){
            throw new FPMallException("更新商品失败");
        }
        ...
        if(tbItemDescMapper.updateByPrimaryKey(tbItemDesc)!=1){
            throw new FPMallException("更新商品详情失败");
        }
        //同步缓存
