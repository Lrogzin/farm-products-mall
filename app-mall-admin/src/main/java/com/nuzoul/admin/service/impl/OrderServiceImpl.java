...
import com.nuzoul.admin.service.OrderService;
...
import com.nuzoul.dto.OrderDetail;
...
import com.nuzoul.mapper.TbThanksMapper;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Long countOrder() {
        ...
        if(result==null){
            throw new FPMallException("统计订单数目失败");
        }
        return result;
    }
    ...
    public int deleteOrder(String id) {
        ...
        if(tbOrderMapper.deleteByPrimaryKey(id)!=1){
            throw new FPMallException("删除订单数失败");
        }
        ...
        if(tbOrder.getStatus()==0){
            ...
            if(days>=1){
                ...
                if(tbOrderMapper.updateByPrimaryKey(tbOrder)!=1){
                    throw new FPMallException("设置订单关闭失败");
                }
            }else {
                //返回到期时间
    ...
    public int backPay(String tokenName, String token, String id) {
        ...
        //发送通知邮箱
        if(StringUtils.isNotBlank(tbThanks.getEmail())&& EmailUtil.checkEmail(tbThanks.getEmail())){
            String content="抱歉，您的订单支付失败，请尝试重新支付！";
        }
        return 1;
    }
