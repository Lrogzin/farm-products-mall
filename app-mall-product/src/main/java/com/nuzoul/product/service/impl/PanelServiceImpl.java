...
import com.nuzoul.common.pojo.ZTreeNode;
...
import com.nuzoul.product.service.PanelService;
...
public class PanelServiceImpl implements PanelService {
    ...
    public TbPanel getTbPanelById(int id) {
        ...
        if(tbPanel==null){
            throw new FPMallException("通过id获得板块失败");
        }
        return tbPanel;
    }
    ...
    public int addPanel(TbPanel tbPanel) {
        ...
        if(tbPanel.getType()==0){
            ...
            if(list!=null&&list.size()>0){
                throw new FPMallException("已有轮播图板块,轮播图仅能添加1个!");
            }
        }
        ...
        if(tbPanelMapper.insert(tbPanel)!=1){
            throw new FPMallException("添加板块失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int updatePanel(TbPanel tbPanel) {
        ...
        if(tbPanelMapper.updateByPrimaryKey(tbPanel)!=1){
            throw new FPMallException("更新板块失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int deletePanel(int id) {
        ...
        if(tbPanelMapper.deleteByPrimaryKey(id)!=1){
            throw new FPMallException("删除内容分类失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
