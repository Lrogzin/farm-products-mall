...
import com.nuzoul.dto.front.AllGoodsResult;
import com.nuzoul.dto.front.Product;
import com.nuzoul.dto.front.ProductDet;
import com.nuzoul.mapper.TbItemDescMapper;
import com.nuzoul.mapper.TbItemMapper;
import com.nuzoul.mapper.TbPanelContentMapper;
import com.nuzoul.mapper.TbPanelMapper;
import com.nuzoul.pojo.*;
import com.nuzoul.product.service.ContentService;
...
public class ContentServiceImpl implements ContentService {
    ...
    public int addPanelContent(TbPanelContent tbPanelContent) {
        ...
        if(tbPanelContentMapper.insert(tbPanelContent)!=1){
            throw new FPMallException("添加首页板块内容失败");
        }
        //同步导航栏缓存
    ...
    public int deletePanelContent(int id) {
        ...
        if(tbPanelContentMapper.deleteByPrimaryKey(id)!=1){
            throw new FPMallException("删除首页板块失败");
        }
        //同步导航栏缓存
    ...
    public int updateContent(TbPanelContent tbPanelContent) {
        ...
        if(tbPanelContentMapper.updateByPrimaryKey(tbPanelContent)!=1){
            throw new FPMallException("更新板块内容失败");
        }
        //同步导航栏缓存
        if(tbPanelContent.getPanelId()==HEADER_PANEL_ID){
            updateNavListRedis();
    ...
    public TbPanelContent getTbPanelContentById(int id) {
        ...
        if(tbPanelContent==null){
            throw new FPMallException("通过id获取板块内容失败");
        }
